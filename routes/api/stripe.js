...
router.get("/secret", async (req, res) => {
  try {
    ...
    transactionLogger.error(
      `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${e}`
    );
  }
});
...
router.get("/getCustomer", async (req, res) => {
  ...
  if (auth) {
    try {
      ...
      console.log(e);
      transactionLogger.error(
        `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${e} - ${auth.email} - ${auth.uid}`
      );
    }
  }
});
...
router.post("/createIntent", async (req, res) => {
  ...
  if (auth) {
    try {
      ...
      if (!user.stripe_id) {
        throw (
          (Error("User doesn't have a stripe id"),
          transactionLogger.error(
            `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - User doesn't have a stripe id - ${auth.email} - ${auth.uid}`
          ))
        );
      }
      ...
      if (setupIntents.data.length === 0) {
        ...
        //Get the intent
        const foundIntent = setupIntents.data.find(
...
router.post("/processPayment", async (req, res) => {
  try {
      ...
      try {
        ...
        if (user && !user.stripe_id) {
          throw (
            (Error("User doesn't have a stripe id"),
            transactionLogger.error(
              `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - User doesn't have a stripe id - Bid: ${auction.bid} - Auction_ID: ${auction.auction_id} - ${user.email} - ${auction.user_id}`
            ))
          );
          ...
          const foundIntent = paymentIntents.data.find(
            ({ status }) => status === "requires_capture"
          );
          // if(!foundIntent) {
          ...
          if (paymentIntent.status === "succeeded") {
            ...
            transactionLogger.info(
              `Payment Successful - ${req.originalUrl} - ${req.method} - Fee(/100): ${paymentIntent.amount_received} - Auction_ID: ${auction.auction_id} - ${user.email} - ${user.uid}`
            );
            //Send email to buyer
            ...
            mailer.transport.sendMail(mailOptions, (error, info) => {
              if (error) {
                ...
                transactionLogger.error(
                  `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${error} - Bid: ${auction.bid} - Auction_ID: ${auction.auction_id} - ${user.email} - ${user.uid}`
                );
              }
              res.status(200).send("Payment sent!");
              transactionLogger.info(
                `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - Email sent successfully - ${user.email} - ${user.uid}`
              );
            });
          }
          res.status(200).json(paymentIntent);
          transactionLogger.info(
            `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - Payment sent successfully - ${user.email} - ${user.uid}`
          );
        } else {
        ...
        transactionLogger.error(
          `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${err} - Bid: ${auction.bid} - Auction_ID: ${auction.auction_id} - ${user.email} - ${user.uid}`
        );
      }
    }
  } catch (err) {
    res.status(400).json(err);
    transactionLogger.error(
      `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${err}`
    );
  }
});
    ...
    try {
      ...
      if (user && !user.stripe_id) {
        ...
        if (paymentIntent.status === "requires_capture") {
          res.status(200).json(paymentIntent);
          transactionLogger.info(
            `Bid Successful - ${req.originalUrl} - ${req.method} - Bid: ${newBid.bid} - Auction_ID: ${newBid.auction_id} - ${auth.email} - ${auth.uid}`
          );
        }
      }
    } catch (err) {
      ...
      transactionLogger.error(
        `${res.statusMessage} - ${req.originalUrl} - ${req.method} - ${req.ip} - ${err} - Bid: ${newBid.bid} - Auction_ID: ${newBid.auction_id} - ${auth.email} - ${auth.uid}`
      );
    }
  }
});
